# Definitive Consensus Guide to Cell-Type and Cell-State Annotation of scRNA-seq/snRNA-seq from a Right-Lung Biopsy in Metastatic Osteosarcoma

**Specimen context:** human right-lung core/wedge biopsy · metastatic conventional osteosarcoma (OS), primary distal femur
**Data assumption:** human scRNA-seq or snRNA-seq; HGNC-style gene symbols
**Purpose:** a single operational reference for cluster annotation — lineage panels, tumor-state programs, discriminators, collision warnings, prior expectations, and validation logic
**Status:** merged synthesis of two independent consensus reports (themselves derived from multiple expert reports). Agreements, residual disagreements, and low-confidence items are flagged explicitly.

> **Scope limitation (unanimous).** Gene signatures support *research* annotation. They are not a diagnostic device. Clinically consequential calls — above all *malignant vs. reactive stroma* and any *epithelial-malignancy differential* — must be reconciled with histopathology, imaging, IHC/IF, and, where available, genomic data.

> **Citation-verification note.** All references supplied by both source reports are preserved in §17. A minority (particularly those with PMIDs in the ≥41,000,000 range and 2025–2026 publication years) could not be cross-verified and are marked **[unverified]**. Treat these as pointers to confirm in PubMed before manuscript or report use.

---

## 1. Why this specimen requires a multi-atlas annotation strategy

A lung biopsy from metastatic osteosarcoma is a **chimeric tissue**. Clusters will fall into four biological families, each requiring a different reference framework. Both source reports converged on this framing.

| Family | Origin | Reference framework | Key refs |
|---|---|---|---|
| **A. Malignant OS cells** | Bone (distal femur) → disseminated | OS single-cell atlases | [1–10, 27–30] |
| **B. Tumor-recruited stroma / immune** | Bone-, lung-, and blood-derived | Pan-cancer TME atlases (T cell, myeloid, CAF, EC) | [74–117] |
| **C. Resident normal lung** | Right lung parenchyma / airway / pleura | Human Lung Cell Atlas (HLCA), Travaglini atlas | [51–65] |
| **D. Injury/fibrosis-reprogrammed lung** | Metastatic niche remodeling | Lung fibrosis + metastatic-niche literature | [25, 66–74] |

**Family D is specific and clinically important here.** Osteosarcoma cells induce acute alveolar epithelial injury and a chronic, non-resolving wound-healing/fibrotic program in the metastasis-bearing lung, with accumulation of partially differentiated (transitional) epithelial intermediates and profibrotic macrophages — a state targetable with nintedanib in preclinical models [25]. If you annotate with a "normal lung" reference only, these transitional epithelial cells will be misassigned (usually to AT2 or basal) or, worse, called malignant.

**Prior expectations specific to a lung metastasis**, from the foundational OS atlas (100,987 cells; 7 primary, 2 recurrent, **2 lung metastatic** lesions) [1]:
- **Enrichment of FABP4⁺ proinflammatory (alveolar-type) macrophages** in lung metastatic lesions.
...
Both reports agree on the single most important operating principle:

> **Do not identify malignant osteosarcoma using collagen/ECM genes alone.**
> `COL1A1`, `COL1A2`, `COL3A1`, `SPARC`, `POSTN`, `FN1`, `THBS2`, `TNC` and related ECM genes are shared by OS cells, normal lung fibroblasts, CAFs, myofibroblasts, and wound-healing/fibrotic mesenchyme.

**Step 1 — QC and ambient-RNA control.**
Filter with sample-aware UMI/gene/mitochondrial thresholds (typical OS scRNA-seq: ~200–6,000 genes/cell, <10–15% mitochondrial reads [5]). In lung tissue, `SFTPC`/`SFTPB`/`SFTPA1`/`SCGB1A1` are extremely abundant ambient transcripts and leak into every cluster; in OS, `COL1A1`/`COL1A2`/`SPARC` soup contaminates immune clusters. Run SoupX or CellBender [126, 129] **before** annotation, and require ≥3 co-expressed markers for any epithelial call.

**Step 2 — Doublet removal.** Scrublet / DoubletFinder or equivalent [127, 128], followed by a manual doublet audit on the collision pairs in §10.

**Step 3 — Level-1 (lineage) gating** with high-specificity parent markers (§3):
`PTPRC` immune · `EPCAM/KRT8/KRT18/KRT19` epithelial · `PECAM1/CDH5/VWF/CLDN5` endothelial · `DCN/LUM/PDGFRA/COL1A1` fibroblast · `RGS5/PDGFRB/CSPG4/NOTCH3` pericyte · `ACTA2/MYH11/CNN1` smooth muscle · `HBB/HBA1` erythroid · `PPBP/PF4` platelet.

**Step 4 — Malignant-cell identification by CNV inference.** This is the single most reliable discriminator between OS cells and normal/reactive fibroblasts. Use inferCNV, CopyKAT, SCEVAN, or the haplotype-aware Numbat [118–125], with T/NK/myeloid cells as the diploid reference. Recurrent OS CNV anchors: **chr8q gain (MYC)**, **chr6p12–p21 amplification (RUNX2, VEGFA)**, **chr17p loss (TP53)**, **chr13q loss (RB1)**, and **chr12q13–15 amplification (MDM2/CDK4)** in a subset [15–17, 43].

**Step 5 — Level-2 (subtype/state) annotation by module scoring.** `AddModuleScore`, UCell, AUCell, or ssGSEA [130]. Never rely on single genes; use ≥5-gene modules with a control-gene background.

**Step 6 — Subcluster each compartment separately** (tumor, immune, epithelial, stromal, vascular) before fine annotation. Tumor states should be annotated *within* CNV-positive malignant cells. Cycling, hypoxia, IFN, and stress are **states, not cell types**.

**Step 7 — Reference mapping as an orthogonal check.** Project onto the integrated HLCA (2.4M cells, 49 datasets, 486 individuals) [53, 62] via Azimuth / scArches / scANVI [131, 132], and onto pan-cancer T-cell [84] and myeloid [85] references.

**Step 8 — Curated-database arbitration** for ambiguous clusters: CellMarker 2.0 (83,361 curated tissue–cell-type–marker entries, 656 tissues) [136, 137], PanglaoDB [138].

**Step 9 — Report a confidence tier per cluster** (high / medium / low; §15) and flag clusters resolved only by CNV or only by a collision-prone gene.

**Step 10 — Validate clinically important calls** with histology/IHC/IF: SATB2, RUNX2, ALPL, osteocalcin (BGLAP), osteoid histology for osteogenic tumor; CD45, CD3, CD68/CD163, CD31, EPCAM/pan-keratin, FAP, αSMA, TRAP (ACP5), CTSK as needed [29–32, 41–51].
...
|---|---|---|---|
| **Malignant OS, osteoblastic** | `SATB2, RUNX2, SP7, ALPL, IBSP, BGLAP, SPP1, COL1A1, COL1A2, COL5A2, SPARC, CDH11, POSTN, COL11A1, OMD, MEPE, MMP13` | CNV⁺; usually `PTPRC⁻`, `PECAM1⁻`, `CDH5⁻`, `EPCAM⁻/low`. **ECM genes alone are insufficient** | [1–10, 29–32] |
| **Malignant OS, chondroblastic** | `SOX9, SOX5, SOX6, ACAN, COL2A1, COL9A1/2/3, COL11A1, COL10A1, COMP, MATN3, CHAD, HAPLN1, CYTL1, WIF1, IHH` | Continuum with osteoblastic OS; diploid cartilage-like cells ≠ malignant | [1, 2, 33, 34] |
| **Osteoclast / multinucleated giant cell** | `CTSK, ACP5, MMP9, ATP6V0D2, DCSTAMP, OCSTAMP, SIGLEC15, CALCR, NFATC1, TNFRSF11A, TCIRG1, CA2, ITGB3, OSCAR` + myeloid (`CD68, CSF1R, TYROBP, PTPRC`) | Myeloid-derived, `PTPRC⁺`; **expect low abundance in lung mets**; do not confuse with `SPP1⁺` tumor cells | [1, 35–40] |
| **Fibroblast / CAF** | `DCN, LUM, COL1A1, COL1A2, COL3A1, COL6A1/2/3, FBLN1, FBLN2, DPT, PDGFRA, C1R, C1S, MMP2, CXCL14` | CNV-neutral; `RUNX2/SP7/IBSP` low | [52, 53, 75–83] |
...
| **Pericyte / mural** | `RGS5, PDGFRB, CSPG4, NOTCH3, MCAM, HIGD1B, KCNJ8, ABCC9, COX4I2, NDUFA4L2`, `ACTA2^low` | `MYH11/CNN1` high favors SMC | [52, 53, 58] |
| **Smooth muscle** | `ACTA2, TAGLN, MYH11, CNN1, DES, ACTG2, LMOD1, SMTN, PLN` | Strong `MYH11/CNN1` favors true SMC | [52, 53] |
| **Endothelial** | `PECAM1, CDH5, CLDN5, VWF, EGFL7, RAMP2, ERG, SOX17, ESAM, KDR, EMCN, ENG` | Subtype within the EC gate | [52, 53, 58–60] |
| **Lung epithelium** | `EPCAM, KRT8, KRT18, KRT19, CDH1, CLDN4, TACSTD2, ELF3, MUC1` | Surfactant RNA is often ambient — require coherent co-expression | [52–57] |
| **Mesothelium (pleura)** | `MSLN, UPK3B, WT1, CALB2, ITLN1, PRG4, LRRN4, PDPN, ALDH1A2, KRT8/18` | Common in pleural-based/wedge biopsies; frequently misannotated | [52, 53, 65] |
| **Myeloid (mono/macro/DC)** | `PTPRC, LYZ, CD68, CD14, AIF1, TYROBP, FCER1G, CSF1R, ITGAM, MNDA, HLA-DRA, CD74` | | [85, 92–96] |
| **Mast cell** | `TPSAB1, TPSB2, CPA3, MS4A2, KIT, CMA1, HPGDS, HDC, VWA5A` | | [52, 61, 85] |
| **Neutrophil / TAN** | `FCGR3B, CSF3R, CXCR2, S100A8, S100A9, S100A12, IL1R2, PROK2, IFITM2, SELL, MPO, ELANE` | Low RNA content, fragile, often lost in 10x | [88, 107–109] |
| **T cell** | `CD3D, CD3E, CD3G, CD2, TRAC, TRBC1, TRBC2, CD247, IL7R` | | [84, 86, 87] |
| **NK cell** | `NKG7, GNLY, KLRD1, KLRF1, PRF1, NCR1, FGFBP2, TYROBP, NCAM1` | `CD3D⁻/TRAC⁻` required | [92, 101] |
| **B cell** | `MS4A1, CD79A, CD79B, CD19, CD22, BANK1, TNFRSF13C, TCL1A, IGHD, IGHM` | `CD74/HLA-DRA` are not B-specific | [92, 103, 104] |
| **Plasma cell** | `MZB1, JCHAIN, DERL3, XBP1, SEC11C, PRDM1, TNFRSF17, SDC1, IGHG1, IGKC, IGLC2` | `PTPRC` can be low; major Ig ambient-RNA source | [92, 103] |
| **Erythroid** | `HBB, HBA1, HBA2, HBD, ALAS2, AHSP` | Usually blood/ambient contamination | [61] |
| **Platelet / megakaryocyte** | `PPBP, PF4, ITGA2B, GP9, TUBB1, NRGN` | Co-expression with another lineage = adherence/doublet | [61] |
...
| **Schwann / peripheral nerve** | `SOX10, S100B, PLP1, MPZ, PMP22, NGFR` | Rare structural component | [52, 61] |
| **Adipocyte (contamination)** | `ADIPOQ, PLIN1, LEP, FABP4, LPL, CFD` | `FABP4` collides with alveolar macrophage | [61, 82] |
| **Cycling (any lineage)** | `MKI67, TOP2A, CCNB1, CENPF, UBE2C, PCNA, TYMS, STMN1, RRM2, CDK1` | Annotate as "cycling ⟨parent lineage⟩", never as its own type | [118] |
...
Apply all signatures in this section **inside the putative malignant compartment**, ideally after CNV inference, and only within `PTPRC⁻ / EPCAM⁻ / PECAM1⁻` cells.
...
`BGLAP` (osteocalcin), `DMP1`, and `SOST` are sparse and dropout-prone in 3′ scRNA-seq. `SATB2` transcript is **supportive, never definitive** (§4.3).

### 4.2 Tumor differentiation states and programs

| State / program | Signature genes | Interpretation / caveats | Refs |
|---|---|---|---|
| **Osteoblastic (OB-like)** | `RUNX2, SP7, SATB2, ALPL, IBSP, BGLAP, SPP1, COL1A1, COL1A2, COL5A1/2, SPARC, MGP, BGN, MEPE, OMD, DLX5, MSX2, PTH1R, CDH11` | Dominant malignant population in most OS by CNV; classical osteoid-producing differentiation | [1, 6, 29–32] |
| **Chondroblastic (CB-like)** | `SOX9, SOX5, SOX6, ACAN, COL2A1, COL9A1/2/3, COL10A1, COL11A1, COL11A2, COMP, MATN3, CHAD, HAPLN1, CYTL1, WIF1, EPYC, SNORC, PTGDS, IHH` | Trajectory/CNV analyses support transdifferentiation of osteoblastic from chondroblastic malignant cells; if **diploid**, consider entrapped benign cartilage | [1, 2, 33, 34] |
| **Fibroblastic / spindle / ECM-rich** | `COL1A1, COL1A2, COL3A1, COL5A1/2, COL6A3, FN1, TNC, THBS2, POSTN, MMP2, MMP11, MMP14, SERPINE1, FAP, PDPN, VCAN, CDH11` | **Highest-risk collision with CAFs.** Requires CNV and/or osteogenic TFs | [1, 2, 75–83] |
| **Proliferating high-grade** | `MKI67, TOP2A, CENPF, UBE2C, CDK1, CCNB1/2, BIRC5, TYMS, RRM2, HMGB2, PCNA, MCM2–7` | Chemo-relevant; often a distinct cluster. Proliferation is not lineage-specific | [5, 118] |
| **MSC-/progenitor-like ("stem-like")** | `CXCL12, SFRP2, MME, THY1, FGF2, NES, PRRX1, SOX4, KLF4, CD44, ALDH1A1, PROM1, MYC` | Candidate tumor-initiating state; overlaps host MSC/fibroblast programs → requires CNV | [1, 2] |
| **UPR / ER-stress** | `ATF6, XBP1, HSPA5, DDIT3, ATF4, HERPUD1, EDEM1, SEL1L, MANF, PDIA4` | UPR-high malignant subcluster with **ATF6α** as top activated TF, associated with aggressiveness (110,000-cell/17-sample OS atlas). Distinguish from dissociation stress | [3] |
...
| **Hypoxic / glycolytic** | `CA9, VEGFA, SLC2A1, LDHA, PDK1, PGK1, ENO1, NDRG1, ADM, BNIP3, HILPDA` | Spatially localized; shared across tumor and non-tumor cells | [1, 75] |
| **Inflammatory / invasive matrix-remodeling** | `CXCL8, IL6, CXCL1, CXCL2, MMP9, MMP13, MMP14, LOX, LOXL2, SERPINE1, PLAUR, ITGA5, TNC, POSTN, COMP, CTHRC1` | `POSTN` strongly upregulated in OS vs. normal osteoblasts (mRNA and protein). Overlaps iCAF and inflammatory myeloid | [5, 75] |
| **IFN / MHC-responsive** | `HLA-A/B/C, B2M, TAP1, NLRC5, STAT1, IRF1, ISG15, IFI6, IFIT1/2/3, MX1, CXCL9, CXCL10`; sometimes `HLA-DRA, CD74` | IFN induction correlated with improved survival in an OS model system. If CNV⁺ and `PTPRC⁻`, consider MHC-II⁺ tumor state rather than APC | [8, 97, 98] |
| **Replication stress** | `CDKN1A, ATR, CHEK1, RPA2, H2AFX, TP53BP1, GADD45A` | Murine hierarchy: multipotent progenitor → immature (replication-stress-enriched) osteoblast → mature osteoblast; p53 loss enables sustained proliferation and **lung metastasis** | [7] |
| **Hedgehog** | `GLI1, PTCH1, HHIP` | Developmental-context module | [7] |
| **Mature osteocyte-like / ossified matrix (rare)** | `DMP1, PHEX, SOST, MEPE, FGF23` | Rarely captured; if diploid, consider entrapped bone; if CNV⁺, terminal osteogenic differentiation | [29–32] |
| **MDM2/CDK4-amplified context** | `MDM2, CDK4, HMGA2` + CNV amplification | Not universal in conventional high-grade OS; relevant if pathology suggests parosteal / low-grade central / dedifferentiated OS | [11, 15–17, 43] |
...
Use a **composite rule** (unanimous across both reports):

1. **CNV positivity** (inferCNV / CopyKAT / SCEVAN / Numbat) — *primary criterion* [118–125].
...
4. **Canonical fibroblast identity genes absent/low:** `DCN, LUM, PDGFRA, FBLN1, CFD, APOD, PI16, SCARA5, NPNT` → these mark CAF/normal fibroblast.
5. **Pathology corroboration, with explicit caveats.** SATB2 is the accepted IHC marker of osteoblastic differentiation [31, 32, 48, 49], but published limitations are substantial:
   - Sensitivity ~92.6% but specificity as low as ~50% in some series; positive in giant cell tumor and fibrous dysplasia [46, 47].
   - Does **not** reliably separate OS from chondrosarcoma on biopsy (SOX9 positive in both) [41].
   - Positive in atypical fibroxanthoma, pleomorphic dermal sarcoma, sarcomatoid SCC/melanoma, leiomyosarcoma [42].
   - Does not distinguish jaw OS from benign fibro-osseous lesions [43]; limitations documented in extraskeletal OS [44].
   - Sensitive/specific when used **in a panel** (canine validation) [45].
   - `SATB2` is also expressed in colorectal epithelium/carcinoma — if `SATB2⁺` cells are `EPCAM/KRT/CDX2⁺`, consider an epithelial metastasis differential instead [31, 32].
...
**Core:** `CTSK, ACP5 (TRAP), MMP9, ATP6V0D2, DCSTAMP, OCSTAMP, SIGLEC15, CALCR, TCIRG1, CA2, NFATC1, ITGB3, OSCAR, TNFRSF11A`
**Myeloid confirmation (required):** `PTPRC, CD68, CSF1R, TYROBP, FCER1G`
**Differentiation axis to score:** progenitor `CD14, CXCR4` → pre-osteoclast `NFATC1, TNFRSF11A (RANK)` → mature `CTSK, ACP5, DCSTAMP, ATP6V0D2, CALCR`; `CD14` is lost during maturation [38, 39, 40].
**Ligand/receptor axis:** `TNFSF11 (RANKL) – TNFRSF11A (RANK) – TNFRSF11B (OPG)`, `CSF1 – CSF1R`, `SPI1`, `FOS` [35–37].

**Caveats:**
- `ATP6V0D2` is shared with **pulmonary ionocytes** — always require `CTSK + ACP5 + MMP9` plus myeloid identity.
- Distinguish osteoclasts from macrophages by high `CTSK/ACP5` and low `CD14`; `CALCR/DCSTAMP` are the most bone-resorption-specific [38, 39].
- **Expect fewer osteoclasts in a lung metastasis than in the femoral primary** [1].
- Osteoclast-differentiation programs and `CSF1R` are broadly expressed across osteoblast and monocyte/macrophage lineages in OS; low-`CSF1R` tumors showed greater predicted ICI sensitivity [27].
...
|---|---|---|---|
| **Pan-epithelial** | `EPCAM, KRT8, KRT18, KRT19, CDH1, CLDN4, TACSTD2, MUC1, ELF3` | Parent gate | [52–55] |
| **AT1** | `AGER, PDPN, CAV1, CAV2, AQP5, HOPX, CLDN18, EMP2, RTKN2, CLIC5, SPOCK2, SCEL, MYRF` | `PDPN` also lymphatic/mesothelial | [52, 53, 60] |
| **AT2** | `SFTPC, SFTPB, SFTPA1, SFTPA2, SFTPD, NAPSA, LAMP3, ABCA3, SLC34A2, PGC, ETV5, LPCAT1, WIF1, HHIP` | Surfactant genes are the dominant ambient RNA in lung — require co-expression | [52, 53] |
| **AT0 / AT2-signaling (novel, rare)** | `SFTPC + SCGB3A2, SFTPB, SCGB3A1, AXIN2, TM4SF1` | Novel state defined in HLCA | [53, 62] |
| **AT2→AT1 transitional** | see §6.2 | Prioritize in this sample | [25, 66–71] |
| **Club / secretory** | `SCGB1A1, SCGB3A1, SCGB3A2, CYP2F1, BPIFA1, BPIFB1, MGP, TFF3, WFDC2, MUC5B` | `SCGB1A1` can be ambient; `WFDC2` non-specific | [52–55] |
| **Goblet / mucous** | `MUC5AC, MUC5B, SPDEF, AGR2, TFF1, TFF3, CLCA1, FCGBP, CEACAM5` | | [52–55] |
| **Ciliated** | `FOXJ1, PIFO, TPPP3, RSPH1, DNAH5, DNAI1, CCDC39, CAPS, SNTN, HYDIN, C20orf85` | Motile cilia program | [52–57] |
| **Basal** | `KRT5, KRT14, KRT15, KRT17, TP63, NGFR, ITGA6, S100A2, DST, MIR205HG, DLK2` | Proximal airway; distinguish from squamous carcinoma by CNV and expansion | [52–55] |
| **Suprabasal / hillock** | `KRT13, KRT4, KRT6A, SCGB3A1, S100A2` | | [53] |
| **Submucosal gland serous / mucous** | `LTF, LYZ, AZGP1, PRR4, ZG16B, BPIFB1, BPIFB2, MUC7` | Rare-cell niche; appears if proximal airway sampled | [53, 55, 64] |
| **Pulmonary ionocyte** | `FOXI1, CFTR, ASCL3, ATP6V1B1, ATP6V1G3, ATP6V0D2, BSND, TMEM61` | ~0.45% of airway epithelium; `ATP6V0D2` collides with osteoclasts | [56, 57, 63] |
| **Tuft / brush** | `POU2F3, ASCL2, TRPM5, DCLK1, AVIL, LRMP, GNAT3, GFI1B, IL25, ALOX5AP` | ~0.002% of airway epithelium; often mislabeled "Other" | [52, 63] |
| **Pulmonary neuroendocrine (PNEC)** | `ASCL1, INSM1, CHGA, CHGB, SYP, NCAM1, CALCA, GRP, SCG2` | If expanded + aneuploid, consider NE carcinoma differential | [52, 53] |

### 6.2 Injury / fibrosis-associated epithelial states — **prioritize in this specimen**

| State | Signature | Why it matters here | Refs |
|---|---|---|---|
| **Transitional AT2 / alveolar differentiation intermediate (ADI) / KRT8-high** | `KRT8^high, KRT18, KRT19, CLDN4, CDKN1A, SFN, KRT7, SPRR1A, SPRR1B, S100A2, LGALS3, LCN2, TACSTD2, MDK, GDF15` + reduced mature AT1/AT2 markers | `SPRR1A` marks murine Krt8⁺ ADIs and human aberrant basaloid cells; ablating Sprr1a⁺ cells reduced fibrosis | [25, 66–69] |
| **Aberrant basaloid (KRT5⁻/KRT17⁺)** | `KRT17^high, KRT5^low/neg, TP63^low, VIM, FN1, ITGB6, CDH2, PRSS2, MMP7, GDF15, SOX9, COL1A1^low` | Co-express basal and mesenchymal markers; two subsets (Krt5^low/Tp63^low vs. Krt5^hi/Tp63^hi) inducible by TGF-β2 | [67, 70, 71] |
...
> **Critical pitfall (both reports).** Aberrant basaloid/ADI cells express `VIM`, `FN1`, collagen transcripts, and `CDKN1A`, and are readily mistaken for **EMT-like malignant OS cells**. Resolve with (i) `EPCAM/KRT8/KRT18/KRT19` positivity, (ii) absence of `RUNX2/SP7/IBSP/ALPL/SATB2`, (iii) CNV-neutral profile.

### 6.3 Epithelial-malignancy differentials (rare but must be excluded)

| Differential | Signature | Refs |
|---|---|---|
| **Lung adenocarcinoma** | `EPCAM, KRT8/18/19, NKX2-1 (TTF-1), NAPSA, SFTPB, SFTPC, MUC1, CEACAM6` + CNV | [49, 50, 51, 90, 91] |
| **Squamous carcinoma** | `TP63, KRT5, KRT6A, KRT6B, KRT14, SOX2, DSG3` + CNV; distinguish from normal basal cells by CNV, expansion, pathology | [50, 90, 91] |
| **Primary pulmonary osteosarcoma** | Osteogenic program + CNV in a lung-primary clinical context | [26] |

TTF-1 plus Napsin-A reach ~99.1% specificity for pulmonary adenocarcinoma, but TTF-1 is not fully specific in isolation [49].
...
|---|---|---|
| **Pan-fibroblast** | `COL1A1, COL1A2, COL3A1, DCN, LUM, DPT, PDGFRA, COL6A1/2/3, C1R, C1S, FBLN1, FBLN2, CXCL14, MMP2, VIM` | [52, 53, 72, 75, 82] |
| **Alveolar fibroblast / lipofibroblast-like** | `NPNT, WNT2, FGF7, FGF10, TCF21, GPC3, LIMCH1, SCN7A, ADH1B, PLIN2, APOE, FABP5, LUM, DCN` | [52, 53, 66, 67, 72] |
| **Adventitial / PI16⁺ fibroblast** | `PI16, DPP4, SCARA5, SERPINF1, MFAP5, CD34, ELN, C7, CFD, CCL11, DPT` | [52, 82] |
| **Peribronchial fibroblast** | `FGF18, WIF1, ASPN, HHIP, FGF7` | [53, 62] |
| **Myofibroblast** | `ACTA2, TAGLN, MYL9, TPM2, ASPN, WIF1, POSTN, FN1, TNC, CTGF, LOX, LOXL2, COL1A1, COL3A1` | [66, 67, 72, 77–81] |
| **myCAF / TGFβ-responsive CAF** | `ACTA2, TAGLN, MYL9, TPM2, FAP, POSTN, CTHRC1, TNC, COL10A1, COL11A1, INHBA, LRRC15, THY1, THBS2, COMP, LOX, ITGA11` | [75, 77–81] |
| **iCAF / inflammatory CAF** | `IL6, CXCL12, CXCL14, CXCL1, CXCL2, CXCL8, CCL2, LIF, HAS1, DPT, APOD, CLU, CFD, C3, C7, PLA2G2A, PDGFRA` | [76–79] |
| **apCAF / antigen-presenting CAF** | `HLA-DRA, HLA-DRB1, HLA-DPA1, HLA-DPB1, CD74, CIITA, SLPI`; `PTPRC⁻` | [78] |
| **Matrix-remodeling CAF** | `COL10A1, COL11A1, COMP, THBS2, MMP2, MMP11, FN1, POSTN, TNC, LOX, CTHRC1` | [75, 80–83] |
| **Fibrotic / metastatic-niche myofibroblast** | `CTHRC1, COL1A1, COL3A1, POSTN, COMP, TNC, FN1, LOX, CDH11, SFRP4` | [25, 72, 73] |

**Consensus caveat:** `COL1A1` is upregulated across *all* mesenchymal subtypes during fibrotic remodeling and is not lineage-discriminating [72, 73]. Use `NPNT` (alveolar), `PI16` (adventitial), `WIF1/FGF18` (peribronchial), and bone TFs (`SATB2/RUNX2`) plus CNV to resolve the COL1A1⁺ space.

### 7.2 Mural cells

- **Pericyte:** `RGS5, NOTCH3, PDGFRB, HIGD1B, KCNJ8, CSPG4, COX4I2, NDUFA4L2, MCAM, ABCC9, TBX5, ACTA2^low`
- **Vascular SMC:** `MYH11, ACTG2, CNN1, DES, LMOD1, TAGLN, ACTA2^high, SMTN`
...
| Subtype | Signature | Refs |
|---|---|---|
| **Pan-EC** | `PECAM1, CDH5, CLDN5, VWF, EGFL7, RAMP2, ERG, SOX17, ESAM, KDR, EMCN, ENG` | [52, 53, 58, 59] |
| **General capillary (gCap)** | `FCN3, CA4, IL7R, EDN1, GPIHBP1, SLC6A4, RGCC, BTNL9, PRX` | [52, 53, 58, 60] |
| **Aerocyte (aCap)** | `HPGD, EDNRB, S100A3, SOSTDC1, IL1RL1, TBX2, TBX3, CHRM2, CA4, APLN` | [52, 53, 60] |
| **Arterial** | `GJA5, DKK2, HEY1, SOX17, EFNB2, SERPINE2, IGFBP3, CXCL12, DLL4, BMX, NOTCH4` | [52, 53, 58] |
| **Venous / venular** | `ACKR1, NR2F2, VCAM1, SELE, SELP, PLVAP, CPE, VWF^high` | [52, 53, 58] |
| **Systemic / bronchial vessel** | `COL15A1, PLVAP, SPRY1, VWA1` | [52, 53, 62] |
| **Lymphatic** | `PROX1, PDPN, LYVE1, FLT4, CCL21, MMRN1, TFF3, TBX1` | [52, 53, 58] |
| **Tip cell / angiogenic (tumor-associated)** | `ESM1, APLN, ANGPT2, CXCR4, DLL4, NID2, PGF, MMP14, KDR^high, FLT1, INSR` | [58, 59, 75] |
| **High endothelial venule (TLS-associated)** | `ACKR1, CCL21, SELP, LTB, CHST4` | [110] |
| **Proliferating EC** | `MKI67 + PECAM1` | [118] |

**Note:** endothelial `INSR` was identified as an OS-associated, prognosis-linked target expressed predominantly in endothelial cells — worth overlaying on EC clusters [28]. Tumor-specific EC states (loss of immune-homing programs, checkpoint co-regulation) are described in lung TME profiling [75].

### 7.4 Other structural cells

- **Mesothelial:** `MSLN, UPK3B, WT1, CALB2, ITLN1, PRG4, LRRN4, PDPN, ALDH1A2, HP, KRT8/18, KRT5^var` [52, 53, 65]
- **Schwann / peripheral nerve (rare):** `SOX10, S100B, PLP1, MPZ, PMP22, NGFR` [52, 61]
- **Adipocyte (rare contamination):** `ADIPOQ, PLIN1, LEP, FABP4, LPL, CFD` — `FABP4` collides with alveolar macrophage [61, 82]

---

## 8. Immune compartment

Adopt the pan-cancer T-cell nomenclature (316 donors, 21 cancer types) [84] and the pan-cancer myeloid taxonomy (210 patients, 15 cancer types) [85], anchored to lung-resident identities from HLCA [53].

### 8.1 T cells

| Subset | Signature | Refs |
|---|---|---|
| **Pan-T** | `CD3D, CD3E, CD3G, CD2, TRAC, TRBC1, TRBC2, CD247, IL7R` | [84, 86] |
| **Naive CD4/CD8** | `CCR7, SELL, TCF7, LEF1, MAL, NOSIP, IL7R, FOXP1` | [84, 113] |
| **Central-memory CD4** | `IL7R, CCR7^mid, ANXA1, AQP3, S100A4, LTB, CD4` (`CD4` also in monocytes — require CD3/TCR) | [84] |
| **Th1 / Th1-like** | `IFNG, TBX21, CXCR3, IL2, GZMK, CCL4, CCL5` | [84] |
| **Th17** | `IL17A, IL17F, RORC, CCR6, KLRB1, IL23R, CTSH, IL26` (usually low abundance) | [84] |
| **Tfh / CXCL13⁺ CD4 (TLS-associated)** | `CXCL13, IL21, BCL6, TOX2, CD200, ICOS, PDCD1, MAF` | [84, 113] |
| **Treg** | `FOXP3, IL2RA, CTLA4, IKZF2, TIGIT, TNFRSF4, TNFRSF9, TNFRSF18, LAYN, BATF, CCR8, IL1R2, ENTPD1` | [84, 105, 113] |
| **Treg — OS-relevant overlay** | + `CXCR4` (CXCR4 inhibition nominated to target OS Tregs) | [20] |
| **GZMK⁺ effector-memory CD8** | `GZMK, GZMA, CCL5, EOMES, CST7, CXCR3, CXCR4, KLRG1, NKG7, CD8A` | [84, 86] |
| **Tissue-resident memory CD8 (Trm)** | `ZNF683, ITGAE (CD103), CXCR6, CD69, ITGA1, XCL1, XCL2, NR4A2` | [84] |
| **Terminally exhausted CD8 (Tex)** | `PDCD1, HAVCR2, LAG3, CTLA4, TIGIT, TOX, LAYN, ENTPD1, CXCL13, GZMB, RBPJ, MYO7A` | [84, 97, 98, 115] |
| **CD8 cytotoxic (general)** | `CD8A, CD8B, NKG7, PRF1, GZMB, GZMH, GNLY, CST7, CCL5` + `CD3D` (distinguishes from NK) | [84] |
| **Effector / Temra** | `FGFBP2, CX3CR1, GZMH, KLRG1, FCGR3A, TBX21, PRF1, NKG7, GNLY` | [84] |
| **ISG⁺ / IFN-responsive T** | `ISG15, IFIT1, IFIT3, MX1, STAT1, OAS1, IFI6` | [84, 115] |
| **Proliferating T** | `MKI67, TOP2A, STMN1, TYMS` | [84] |
| **MAIT** | `SLC4A10, KLRB1, ZBTB16, NCR3, TRAV1-2, RORC, IL7R, DPP4, IL18RAP, CEBPD` | [84] |
| **γδ T** | `TRDC, TRGC1, TRGC2, TRDV2, TRGV9, KLRC1, NKG7, CD3D` | [84] |
...
> **Caveat:** activation can transiently induce checkpoint genes; interpret exhaustion calls with state context and clonality where available.

### 8.2 NK cells and ILCs

| Subset | Signature | Refs |
|---|---|---|
| **CD56^dim cytotoxic NK** | `FGFBP2, FCGR3A, KLRF1, GNLY, PRF1, GZMB, SPON2, CX3CR1, KLRD1, S1PR5, TYROBP, NKG7, NCR1`; `CD3D⁻/TRAC⁻` | [92, 101] |
| **CD56^bright / tissue-resident NK** | `NCAM1, XCL1, XCL2, GZMK, SELL, KLRC1, CD160, ITGA1, IL7R, IL18` | [101] |
| **ILC / ILC2-like (rare)** | `IL7R, RORA, GATA3, IL1RL1, KIT, KLRB1, AHR, IL23R`; `CD3D⁻` | [52, 53, 61] |
| **NK-ligand overlay (CAR-NK/GD2 programs)** | `ULBP1-3, MICA, MICB, PVR, NECTIN2, HLA-E, NCR3LG1 (B7-H6)` on tumor cells | [144] |

### 8.3 B cells, plasma cells, and TLS

| Subset | Signature | Refs |
|---|---|---|
| **Naive B** | `MS4A1, IGHD, IGHM, TCL1A, FCER2, IL4R, CD79A/B, BANK1, BACH2, CD19, CD22` | [92, 103] |
| **Memory B** | `MS4A1, CD27, TNFRSF13B, AIM2, CD24, IGHG1` | [103, 104] |
| **Germinal-center B** | `AICDA, RGS13, MEF2B, LMO2, S1PR2, BCL6, MKI67` | [61, 104] |
| **Atypical / CD11c⁺ B** | `ITGAX, TBX21, FCRL5, ZEB2` — enriched in lung tumors vs. normal lung/blood, localize near CD4⁺ T cells | [111] |
| **Plasmablast** | `MZB1, JCHAIN + MKI67, TOP2A` | [103] |
| **Plasma cell** | `MZB1, JCHAIN, DERL3, XBP1, SEC11C, PRDM1, TNFRSF17, SDC1, IGHG1-4, IGKC, IGLC2` | [92, 103] |
| **TLS module (neighborhood-level)** | `CXCL13, CCL19, CCL21, CR2, LTB, CXCR5, SELL, MS4A1, LAMP3, CCR7, PDCD1LG2` | [110, 112] |

Prefer spatial/neighborhood interpretation for TLS if spatial data are available.

### 8.4 Myeloid cells

| Subset | Signature | Notes | Refs |
|---|---|---|---|
| **Classical monocyte** | `CD14, S100A8, S100A9, S100A12, VCAN, FCN1, LYZ, SELL, CCR2, CD300E, LILRB2` | Can resemble monocytic MDSC-like states | [85, 92] |
| **Non-classical / patrolling monocyte** | `FCGR3A, CX3CR1, LST1, MS4A7, CDKN1C, LILRB1/2, TCF7L2, IFITM3, RHOC` | Distinguish from NK by myeloid genes and lack of `NKG7/GNLY` dominance | [85, 92] |
| **Pan-macrophage** | `C1QA, C1QB, C1QC, APOE, CD68, CSF1R, AIF1, MRC1, CD163, LYZ, TYROBP` | | [85, 93–96] |
| **Alveolar macrophage (resident)** | `FABP4, MARCO, MCEMP1, PPARG, MSR1, MRC1, LPL, CIDEC, OLR1, SERPING1, SIGLEC1, APOC1, APOE` | **FABP4⁺ proinflammatory macrophages specifically enriched in lung-metastatic OS** — expect this cluster to be prominent | [1, 53, 88] |
| **Interstitial / FOLR2⁺ perivascular macrophage** | `C1QA/B/C, FOLR2, LYVE1, SELENOP, F13A1, MAF, MAFB, CCL13, MS4A7, MRC1` | Less `MARCO/FABP4` | [53, 85] |
| **SPP1⁺/TREM2⁺ lipid-associated profibrotic TAM** | `SPP1, TREM2, GPNMB, APOE, APOC1, CD9, LGALS3, FABP5, MMP9, MMP12, CHI3L1, ACP5, CTSB, CTSD, CTSL, LIPA` | HLCA identified SPP1⁺ profibrotic monocyte-derived macrophages as a **shared state across COVID-19, pulmonary fibrosis, and lung carcinoma**; SPP1–integrin/CD44 signaling to CAFs promotes invasion/metastasis/immune evasion; prognostic in multiple cancers | [53, 106, 114] |
| **CXCL9/10⁺ IFN-TAM ("M1-like")** | `CXCL9, CXCL10, CXCL11, GBP1, GBP5, STAT1, IDO1, ISG15, IFIT1, IFIT3, MX1, CD80` | Separate from DC/B/apCAF/tumor by parent markers | [85, 113] |
| **Inflammatory macrophage** | `IL1B, CXCL8, CCL3, CCL4, TNF, NLRP3, PTGS2, S100A8, S100A9` | Overlaps classical monocytes and dissociation stress | [85, 96] |
| **Angiogenic / pro-tumor TAM** | `VEGFA, VCAN, THBS1, HIF1A, SLC2A1, EREG, INHBA, CD300E, SPP1` | **Pro-angiogenic TAM markers are cancer-type-diverse — do not assume transferability** | [85] |
| **"M2" / immunoregulatory TAM** | `CD163, MRC1, MSR1, STAB1, MAF, TGFB1, IL10, VSIG4` | M1/M2 dichotomy is insufficient in vivo; subsets co-express signatures | [85, 95] |
| **MHC-II^high antigen-presenting macrophage** | `HLA-DRA, HLA-DRB1, HLA-DPA1/B1, CD74, CD86, CIITA` + macrophage genes | | [85, 92–95] |
| **OS-nominated TAM states** | `TXNIP`⁺; `IFIT1/IFIT3/ISG15`⁺; trajectory `SLC40A1 → MT1G → CXCL10` | Reported in OS scRNA-seq syntheses | [4, 20] |
| **cDC1** | `CLEC9A, XCR1, CADM1, BATF3, IRF8, WDFY4, CLNK, IDO1` | Cross-presenting DC | [85, 92–94] |
| **cDC2** | `CD1C, FCER1A, CLEC10A, CD1E, CD1A, HLA-DQA1, FCGR2B, IRF4, ITGAX` | | [85, 92–94] |
| **LAMP3⁺ mregDC (mature/migratory)** | `LAMP3, CCR7, CD40, CD83, FSCN1, IL4I1, IDO1, CCL19, CCL17, CCL22, BIRC3, CD274, PDCD1LG2` | Derives from both cDC1 and cDC2; maps to TLS-proximal niches; `LAMP3` collides with AT2 | [85, 110] |
| **pDC** | `LILRA4, CLEC4C, IL3RA, GZMB, JCHAIN, IRF7, TCF4, SERPINF1, LAMP5` | Often low abundance | [85, 92–94] |
| **Langerhans-like DC** | `CD207, CD1A` | | [85] |
| **Monocytic MDSC-like** | `S100A8, S100A9, S100A12, CD14, IL1B, VEGFA, OLR1, CD84, TREM1` | State, not a clean lineage | [85, 96] |
| **Tumor-associated neutrophil (TAN) / PMN-MDSC-like** | `FCGR3B, CSF3R, CXCR2, IL1R2, MMP9, PROK2, S100A12, IFITM2, CD83, SELL, NAMPT, LCN2, CEACAM8, MPO, ELANE, ARG1^low` | Continuum of states (pro-tumor, inflammatory, ISG-high, antigen-presenting) rather than discrete lineages; `CD83` proposed as a hallmark of senescent pro-tumor TANs; severe mRNA–protein discordance | [107–109] |
| **Eosinophil** | `CLC, PRG2, RNASE2, RNASE3, IL5RA, CCR3, SIGLEC8` | Rare but interpretable | [61, 92] |
| **Basophil** | `MS4A2, FCER1A, IL3RA, HDC, GATA2, CLC`; `KIT^low`, `TPSB2^low` | Distinguish from mast cells | [61, 92] |
| **Mast cell** | `TPSAB1, TPSB2, CPA3, MS4A2, KIT, CMA1, HPGDS, HDC, VWA5A` | **Reported to increase in metastatic vs. primary OS**; TNF⁺/VEGFA⁺ ratio has prognostic relevance | [4, 85] |
...
| **Cell cycle (S/G2M)** | `MKI67, TOP2A, UBE2C, CENPF, CDK1, CCNB1, CCNB2, BIRC5, TYMS, PCNA, STMN1, RRM2, MCM2–7` | Regress or label "cycling ⟨lineage⟩" [118] |
...
| **Ambient surfactant RNA** | `SFTPA1, SFTPA2, SFTPB, SFTPC, SFTPD` | Require epithelial co-expression for AT2 calls [126] |
| **Ambient immunoglobulin RNA** | `IGHG*, IGHA*, IGKC, IGLC*, JCHAIN` | Check full plasma-cell program |
| **Ambient hemoglobin / RBC** | `HBA1, HBA2, HBB, HBD, ALAS2, AHSP` | Blood contamination or erythroid cells |
| **Ambient tumor collagen (OS-specific)** | `COL1A1, COL1A2, SPARC` | Contaminates immune clusters |
| **Platelet adherence** | `PPBP, PF4, GP9, ITGA2B, TUBB1, NRGN` | Co-expression with another lineage → adherence/doublet |
| **Doublet red flags** | `PTPRC + EPCAM`; `EPCAM + COL1A1`; `PECAM1 + CD3D`; `COL1A1 + CD3D`; `PECAM1 + LYZ`; `PPBP/PF4 + any lineage` | Review UMI counts and doublet scores [127–129] |

---

## 10. High-priority marker-collision warnings

Unanimous across both reports; these are the dominant error sources in this specimen.

| Gene(s) | Collides across | How to resolve |
|---|---|---|
| `COL1A1, COL1A2, COL3A1, SPARC, POSTN, FN1, THBS2, TNC` | Malignant OS · fibroblasts · CAFs · myofibroblasts · fibrotic mesenchyme | CNV + `SATB2/RUNX2/SP7/ALPL/IBSP/BGLAP` vs. `DCN/LUM/PDGFRA/PI16/NPNT` |
| `SPP1` | Malignant OS · SPP1⁺/TREM2⁺ TAM · injury AT2 epithelium · CAF | Parent lineage markers: `PTPRC/CSF1R/C1Q` (myeloid) vs. `EPCAM/KRT/SFTPC` (epithelial) vs. CNV + osteogenic (tumor) |
| `LAMP3` | AT2 epithelium · mature/migratory DC (mregDC) | `EPCAM/SFTPC/ABCA3` vs. `PTPRC/HLA-DRA/CCR7/FSCN1` |
| `HLA-DRA, CD74` | B cells · macrophages · DCs · apCAFs · MHC-II⁺ tumor | `PTPRC` + lineage markers + fibroblast markers + CNV |
...
| `SATB2` | Osteoblastic OS · colorectal epithelium/carcinoma · several sarcomas/carcinomas by IHC | `EPCAM/KRT/CDX2⁺` → epithelial differential; see §4.3 caveats [41–47] |
| `ATP6V0D2` | Osteoclast · pulmonary ionocyte | Osteoclast: `CTSK + ACP5 + MMP9 + CSF1R/CD68`; ionocyte: `FOXI1/CFTR/ASCL3` |
| `KRT17` | Basal cells · aberrant basaloid injury epithelium · squamous carcinoma | `KRT5/TP63` level, `VIM/FN1/ITGB6` co-expression, CNV, pathology |
| `FABP4` | Alveolar macrophage · adipocyte · some EC/stromal contexts | `PTPRC/C1Q/MARCO/PPARG` vs. `ADIPOQ/PLIN1/LEP` vs. `PECAM1` |
| `PDPN` | AT1 · lymphatic EC · mesothelium · fibroblast/CAF | Parent gate (`EPCAM/AGER` vs. `PROX1/LYVE1` vs. `MSLN/WT1` vs. `DCN/LUM`) |
| `FCGR3A` | NK · non-classical monocyte · macrophage | `NKG7/GNLY/KLRF1` + absent TCR vs. `LYZ/LST1/CSF1R` |
| `S100A8/A9` | Neutrophil · classical monocyte · MDSC-like · inflammatory macrophage | `FCGR3B/CSF3R/CXCR2` vs. `CD14/VCAN/FCN1` |
| `SOX9` | Chondroblastic OS · chondrosarcoma · aberrant basaloid epithelium | Does not separate OS from chondrosarcoma on biopsy [41]; use `EPCAM/KRT` and CNV for the epithelial differential |
| `CD4` | CD4 T cells · monocytes/macrophages | Require `CD3D/CD3E/TRAC` |
...
| Malignant chondroblastic OS | Variable; likely if primary had a chondroblastic component | [1] |
...
| Myeloid overall | **Decreased** proportion vs. primary, but with **FABP4⁺ alveolar macrophage enrichment** and SPP1⁺/TREM2⁺ profibrotic TAMs | [1, 4, 53] |
| Resident lung epithelium (AT1/AT2/club/ciliated) | Present; proportion depends on tumor:normal ratio in the core | [52, 53] |
| Transitional / aberrant basaloid epithelium | Present at the metastasis interface | [25, 69] |
| Myofibroblasts / fibrosis program | Present (metastasis-induced fibrosis) | [25] |
| Mesothelium | Present if pleural-based/wedge sampling | [65] |
| Skeletal myoblasts | Should be **absent** (bone/limb-specific cluster) | [1] |

Deviations from these priors are themselves informative and should be reported explicitly.

---

## 12. Clinically actionable gene overlay (report per cluster; do **not** use for annotation)

| Target axis | Genes | Cell types to quantify | Refs |
|---|---|---|---|
| **B7-H3 / CD276** | `CD276` | Malignant OS (primary), TAMs | [139, 142, 143] |
| **B7-H3 CAR-T homing** | Tumor `CXCL8, CXCL1, CXCL2, CXCL16`; T-cell `CXCR2, CXCR6` | Malignant OS, T cells | [140, 141] |
| **GD2** | `B4GALNT1, ST8SIA1` (synthases; GD2 itself is a glycolipid) | Malignant OS | [144] |
| **Checkpoints** | `PDCD1, CTLA4, LAG3, HAVCR2, TIGIT, CD274, PDCD1LG2, VSIR, BTLA` | T/NK, TAM, malignant | [1, 20, 97, 98] |
| **Myeloid targeting** | `CSF1R, CSF1, CCR2, CCL2, TREM2, MARCO, SIRPA, CD47` | TAM, monocyte, osteoclast | [27, 106] |
| **Bone-directed therapy** | `TNFSF11 (RANKL), TNFRSF11A, TNFRSF11B` | Osteoblastic OS, osteoclasts, MSC | [20, 35] |
| **Antiangiogenic TKI targets** | `KDR, FLT1, FLT4, PDGFRA, PDGFRB, FGFR1, MET, RET, KIT, INSR` | EC, pericyte, malignant | [28, 25] |
...
| **Amplification-linked** | `MDM2, CDK4, MYC, RUNX2, VEGFA` | Malignant OS | [15–17, 43] |
| **Fibrosis / niche (nintedanib rationale)** | `FN1, TGFB1, TGFB2, ITGB6, LOX, SERPINE1, CTHRC1` + FGFR/PDGFR/VEGFR | ADI/ABC epithelium, myofibroblast, malignant | [25, 70] |
...
sigs <- list(
  ...
  OS_chondroblastic  = c("SOX9","SOX5","SOX6","ACAN","COL2A1","COL9A1","COL9A2","COL9A3","COL10A1",
                         "COL11A1","COL11A2","COMP","MATN3","CHAD","HAPLN1","CYTL1","WIF1","EPYC","SNORC"),
  OS_fibroblastic_ecm= c("COL1A1","COL1A2","COL3A1","COL5A1","COL5A2","COL6A3","FN1","TNC","THBS2",
                         "POSTN","MMP2","MMP11","MMP14","SERPINE1","FAP","PDPN","VCAN","CDH11"),
  OS_proliferating   = c("MKI67","TOP2A","CENPF","UBE2C","CDK1","CCNB1","CCNB2","BIRC5","TYMS",
                         "RRM2","HMGB2","PCNA","MCM2","MCM3","MCM4","MCM5","MCM6","MCM7"),
  OS_MSClike         = c("CXCL12","SFRP2","MME","THY1","FGF2","NES","PRRX1","SOX4","KLF4","CD44",
                         "ALDH1A1","PROM1","MYC"),
  ...
  OS_invasive_matrix = c("CXCL8","IL6","CXCL1","CXCL2","MMP9","MMP13","MMP14","LOX","LOXL2",
                         "SERPINE1","PLAUR","ITGA5","TNC","POSTN","COMP","CTHRC1"),
  OS_IFN_MHC         = c("HLA-A","HLA-B","HLA-C","B2M","TAP1","NLRC5","STAT1","IRF1","ISG15",
                         "IFI6","IFIT1","IFIT2","IFIT3","MX1","CXCL9","CXCL10"),
  OS_replstress      = c("CDKN1A","ATR","CHEK1","RPA2","H2AFX","TP53BP1","GADD45A"),
  OS_hedgehog        = c("GLI1","PTCH1","HHIP"),
  OS_osteocyte_like  = c("DMP1","PHEX","SOST","MEPE","FGF23"),
  OS_MDM2_CDK4       = c("MDM2","CDK4","HMGA2"),

  # ---------------- Bone lineage / osteoclast ----------------
  Osteoclast         = c("CTSK","ACP5","MMP9","ATP6V0D2","DCSTAMP","OCSTAMP","SIGLEC15","CALCR",
                         "TNFRSF11A","TCIRG1","CA2","NFATC1","ITGB3","OSCAR"),
  MSC_osteoprog      = c("CXCL12","SFRP2","MME","THY1","ENG","NT5E","LEPR","GREM1","KITLG","PDGFRB"),

  # ---------------- Stroma ----------------
  Fibroblast_pan     = c("DCN","LUM","COL1A1","COL1A2","COL3A1","COL6A1","COL6A3","FBLN1","FBLN2",
                         "DPT","PDGFRA","C1R","C1S","CXCL14","MMP2"),
  Fib_alveolar       = c("NPNT","WNT2","FGF10","FGF7","TCF21","GPC3","LIMCH1","SCN7A","ADH1B","PLIN2"),
  Fib_adventitial    = c("PI16","DPP4","SCARA5","SERPINF1","MFAP5","CD34","ELN","C7","CFD","CCL11","DPT"),
  Fib_peribronchial  = c("FGF18","WIF1","ASPN","HHIP","FGF7"),
  ...
  CAF_myCAF          = c("ACTA2","TAGLN","MYL9","TPM2","FAP","POSTN","CTHRC1","TNC","COL10A1",
                         "COL11A1","INHBA","LRRC15","ITGA11","THY1","THBS2","COMP","LOX"),
  ...
  CAF_matrixremodel  = c("COL10A1","COL11A1","COMP","THBS2","MMP2","MMP11","FN1","POSTN","TNC","LOX","CTHRC1"),
  ...
  Mesothelial        = c("MSLN","UPK3B","WT1","CALB2","ITLN1","PRG4","LRRN4","PDPN","ALDH1A2","KRT8","KRT18"),
  Schwann            = c("SOX10","S100B","PLP1","MPZ","PMP22","NGFR"),
  Adipocyte          = c("ADIPOQ","PLIN1","LEP","FABP4","LPL","CFD"),

  # ---------------- Endothelium ----------------
  EC_pan             = c("PECAM1","CDH5","CLDN5","VWF","EGFL7","RAMP2","ERG","SOX17","ESAM","KDR","EMCN","ENG"),
  EC_gCap            = c("FCN3","CA4","IL7R","EDN1","GPIHBP1","SLC6A4","RGCC","BTNL9","PRX"),
  EC_aerocyte        = c("HPGD","EDNRB","S100A3","SOSTDC1","IL1RL1","TBX2","TBX3","APLN"),
  EC_arterial        = c("GJA5","DKK2","HEY1","SOX17","EFNB2","SERPINE2","IGFBP3","DLL4","BMX","NOTCH4"),
  ...
  EC_tip_angiogenic  = c("ESM1","APLN","ANGPT2","CXCR4","DLL4","NID2","PGF","MMP14","FLT1","INSR"),
  ...
  Club               = c("SCGB1A1","SCGB3A1","SCGB3A2","CYP2F1","BPIFA1","BPIFB1","MGP","TFF3","WFDC2","MUC5B"),
  Goblet             = c("MUC5AC","MUC5B","SPDEF","AGR2","TFF1","TFF3","CLCA1","FCGBP","CEACAM5"),
  Ciliated           = c("FOXJ1","PIFO","TPPP3","RSPH1","DNAH5","DNAI1","CCDC39","CAPS","SNTN","C20orf85"),
  Basal              = c("KRT5","KRT14","KRT15","KRT17","TP63","NGFR","ITGA6","S100A2","DST","MIR205HG","DLK2"),
  Hillock            = c("KRT13","KRT4","KRT6A","SCGB3A1","S100A2"),
  SMG_serous         = c("LTF","LYZ","AZGP1","PRR4","ZG16B","MUC7","BPIFB1","BPIFB2"),
  Ionocyte           = c("FOXI1","CFTR","ASCL3","ATP6V1B1","ATP6V1G3","BSND","TMEM61"),
  Tuft               = c("POU2F3","ASCL2","TRPM5","DCLK1","AVIL","LRMP","GNAT3","GFI1B","IL25"),
  PNEC               = c("ASCL1","INSM1","CHGA","CHGB","SYP","NCAM1","CALCA","GRP","SCG2"),
  Epi_ADI_KRT8       = c("KRT8","KRT18","KRT19","CLDN4","CDKN1A","SFN","KRT7","SPRR1A","SPRR1B",
                         "S100A2","LGALS3","LCN2","TACSTD2","MDK","GDF15"),
  Epi_aberrantBasal  = c("KRT17","VIM","FN1","ITGB6","CDH2","PRSS2","MMP7","GDF15","SOX9","TP63"),
  Epi_metNichFibro   = c("FN1","TNC","SERPINE1","TGFB1","TGFB2","ITGB6"),
  ...
  Mono_classical     = c("CD14","S100A8","S100A9","S100A12","VCAN","FCN1","LYZ","SELL","CCR2","CD300E","LILRB2"),
  Mono_nonclassical  = c("FCGR3A","CX3CR1","LST1","MS4A7","CDKN1C","LILRB1","LILRB2","TCF7L2","IFITM3","RHOC"),
  Mac_pan            = c("C1QA","C1QB","C1QC","APOE","CD68","CSF1R","AIF1","MRC1","CD163","LYZ","TYROBP"),
  Mac_alveolar       = c("FABP4","MARCO","MCEMP1","PPARG","MSR1","MRC1","LPL","CIDEC","OLR1",
                         "SERPING1","SIGLEC1","APOC1","APOE"),
  ...
  TAM_SPP1_TREM2     = c("SPP1","TREM2","GPNMB","APOE","APOC1","CD9","LGALS3","FABP5","MMP9",
                         "MMP12","CHI3L1","ACP5","CTSB","CTSD","CTSL","LIPA"),
  TAM_IFN            = c("CXCL9","CXCL10","CXCL11","GBP1","GBP5","STAT1","IDO1","ISG15","IFIT1","IFIT3","MX1","CD80"),
  TAM_inflammatory   = c("IL1B","CXCL8","CCL3","CCL4","TNF","NLRP3","PTGS2"),
  TAM_angiogenic     = c("VEGFA","VCAN","THBS1","HIF1A","SLC2A1","EREG","INHBA","CD300E","SPP1"),
  TAM_M2like         = c("CD163","MRC1","MSR1","STAB1","MAF","TGFB1","IL10","VSIG4"),
  TAM_MHCII          = c("HLA-DRA","HLA-DRB1","HLA-DPA1","HLA-DPB1","CD74","CD86","CIITA"),
  cDC1               = c("CLEC9A","XCR1","CADM1","BATF3","IRF8","WDFY4","CLNK","IDO1"),
  cDC2               = c("CD1C","FCER1A","CLEC10A","CD1E","CD1A","FCGR2B","IRF4","ITGAX"),
  ...
  Langerhans_like    = c("CD207","CD1A"),
  MDSC_monocytic     = c("S100A8","S100A9","S100A12","CD14","IL1B","VEGFA","OLR1","CD84","TREM1"),
  ...
  Neutrophil_TAN     = c("FCGR3B","CSF3R","CXCR2","IL1R2","MMP9","PROK2","S100A12","IFITM2",
                         "CD83","SELL","LCN2","CEACAM8","MPO","ELANE"),

  # ---------------- Lymphoid ----------------
  Tcell_pan          = c("CD3D","CD3E","CD3G","CD2","TRAC","TRBC1","TRBC2","CD247","IL7R"),
  T_naive            = c("CCR7","SELL","TCF7","LEF1","MAL","NOSIP","IL7R","FOXP1"),
  CD4_Tcm            = c("IL7R","ANXA1","AQP3","S100A4","LTB","CD4"),
  ...
  CD8_cytotoxic      = c("CD8A","CD8B","NKG7","PRF1","GZMB","GZMH","GNLY","CST7","CCL5"),
  ...
  MAIT               = c("SLC4A10","KLRB1","ZBTB16","NCR3","TRAV1-2","RORC","DPP4","IL18RAP","CEBPD"),
  gdT                = c("TRDC","TRGC1","TRGC2","TRDV2","TRGV9","KLRC1","NKG7","CD3D"),
  NK_cytotoxic       = c("FGFBP2","FCGR3A","KLRF1","GNLY","PRF1","GZMB","SPON2","CX3CR1","KLRD1",
                         "S1PR5","NCR1","NKG7"),
  NK_resident        = c("NCAM1","XCL1","XCL2","GZMK","SELL","KLRC1","CD160","ITGA1","IL7R","IL18"),
  ILC                = c("IL7R","RORA","GATA3","IL1RL1","KIT","KLRB1","AHR","IL23R"),
  B_naive            = c("MS4A1","IGHD","IGHM","TCL1A","FCER2","IL4R","CD79A","CD79B","BANK1","CD19","CD22"),
  ...
  TLS_module         = c("CXCL13","CCL19","CCL21","CR2","LTB","CXCR5","SELL","MS4A1","LAMP3","CCR7","PDCD1LG2"),
  ...
  Cycling            = c("MKI67","TOP2A","CCNB1","CCNB2","CENPF","UBE2C","PCNA","TYMS","STMN1","RRM2","CDK1"),
  ...
  Ambient_surfactant = c("SFTPA1","SFTPA2","SFTPB","SFTPC","SFTPD"),
  Ambient_Ig         = c("IGKC","IGHG1","IGHA1","JCHAIN")
)
```

---

## 14. Suggested cluster-level deliverable
...
top_20_DE_genes
broad_lineage_call
best_signature_score (score, z)
second_best_signature_score
CNV_status
reference_mapping_label (HLCA / pan-cancer T / pan-cancer myeloid)
final_annotation
...
## 15. Confidence rules — when to flag "requires orthogonal validation"

Flag a cluster if **any** of the following hold:

- (a) Top and second-best module scores differ by <1 SD (i.e., the two best signature scores are close).
- (b) CNV status is ambiguous or the malignant call rests on CNV alone.
- (c) The annotation depends heavily on a single collision-prone gene: `SPP1`, `LAMP3`, `ACTA2`, `SATB2`, `KRT17`, `ATP6V0D2`, `FABP4`, `PDPN`, `SOX9`.
- (d) The cluster is defined by a **state** (proliferation, IFN, hypoxia, UPR, dissociation stress) rather than a lineage.
- (e) The cluster co-expresses markers from incompatible parent lineages: `PTPRC + EPCAM`, `CD3D + PECAM1`, `COL1A1 + EPCAM`, `COL1A1 + CD3D`, `PECAM1 + LYZ`, or `PPBP/PF4` with a strong non-platelet lineage.

---

## 16. Consensus caveats and limitations

1. **No single gene is definitive.** Use ≥5-gene modules with a permutation/control-gene background (UCell/AUCell) and report score distributions rather than binary calls [130].
...
3. **ECM/collagen genes never identify malignancy on their own** — the single most repeated warning in both source reports.
4. **Cell-state modules are not cell types.** Proliferation, hypoxia, IFN, UPR, and dissociation-stress modules appear across lineages; score them *within* an annotated lineage.
5. **Pro-angiogenic TAM markers are cancer-type-specific** and may not transfer from other tumors [85].
6. **M1/M2 is insufficient in vivo** — subsets co-express both signatures [85, 95].
7. **Neutrophils are systematically under-captured** and transcriptomically shallow, with severe mRNA–protein discordance; consider protein-based or CITE-seq confirmation [107–109].
8. **SATB2/SOX9 IHC caveats apply equally at the transcript level** (§4.3) [41–47].
9. **Prefer consortium-level references** (HLCA, pan-cancer T/myeloid atlases) over ad hoc marker lists from small single-sample studies; several cited OS studies are hypothesis-generating (e.g., analyses based on a single public GSM from GSE152048) [5].
10. **Chemotherapy effects and sample-size limits** alter OS transcriptional profiles; metastasis-associated hub genes require clinical validation [1, 9, 10].
11. **Ambient RNA is the single most common cause of false-positive epithelial and tumor calls in lung samples** — decontaminate *before*, not after, annotation [126, 129].
12. **snRNA-seq caveats.** If nuclei rather than whole cells were profiled, expect depletion of cytoplasmic transcripts (e.g., surfactant, hemoglobin, mitochondrial genes) and relative loss of granulocytes; adjust module expectations accordingly.
13. **Research use only. Not a diagnostic device.** Clinically consequential cell-identity calls must be corroborated by histopathology, IHC, and, where indicated, orthogonal molecular testing.

---

## 17. Consolidated references

*(Merged and deduplicated across both source reports and their upstream bibliographies. Entries marked **[unverified]** could not be cross-verified and should be confirmed before citation. Items after mid-2024 and tool/web resources should be verified against the final manuscript or database record before formal publication use.)*
...
### III. Diagnostic marker validity (SATB2, SOX9, MDM2, TTF-1) and lung tumor classification
...
60. Gillich A, Zhang F, Farmer CG, et al. **Capillary cell-type specialization in the alveolus.** *Nature.* 2020;586:785–790.
...
126. Young MD, Behjati S. **SoupX removes ambient RNA contamination from droplet-based single-cell RNA sequencing data.** *GigaScience.* 2020;9:giaa151. PMC7763177.
127. Wolock SL, Lopez R, Klein AM. **Scrublet: computational identification of cell doublets in single-cell transcriptomic data.** *Cell Syst.* 2019;8:281–291.e9. PMC6625319.
...
130. Andreatta M, Carmona SJ. **UCell: robust and scalable single-cell gene signature scoring.** *Comput Struct Biotechnol J.* 2021;19:3796–3798. PMC8271111. (Also: **UCell and pyUCell.** *Bioinformatics.* 2026.)
