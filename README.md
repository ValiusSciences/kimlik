...
It queries three LLM providers in parallel, has two of them independently consolidate the results, then merges those into a single definitive guide — every step with citations to peer-reviewed literature.
...
Model names change faster than this tool does, so every model is overridable — you should never need to edit source to move to a newer generation. Each setting resolves in this order: **CLI flag → environment variable → built-in default.**
...
The models used are printed at startup and recorded in `kimlik_state.json`, so a finished run always states what produced it. Token budgets follow the *phase*, not the model name — overriding a model never silently changes the output ceiling.
...
A complete run is committed under [`results/example-osteosarcoma-lung-met/`](results/example-osteosarcoma-lung-met/) so you can read real output before spending an API budget. It uses the metastatic case described above — a synthetic, textbook presentation, not a real patient:
...
Changing models between resumes is allowed — useful for retrying one failed provider on a different model. The tool prints a notice when the current models differ from those recorded, and outputs already on disk keep whichever model produced them.
...
The poll budget is 7200 s (`_POLL_TIMEOUT_SECONDS` in `parallel_provider.py`). The tool retries polling automatically across restarts — just re-run the same command. Check the active `task_id` in `kimlik_state.json` if you want to monitor it on the Parallel.ai dashboard.
